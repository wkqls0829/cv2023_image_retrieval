    ...
    def __call__(self, batch, labels, return_distances=False):
        ...
        for i in range(bs):
            ...
            if np.sum(pos)>1:
                ...
                if labels[i] in self.tail:                    
                    indices_in_tail = (np.where(np.isin(labels[neg], self.tail)))
                    neg_mask = np.zeros_like(labels, dtype=bool)
                    neg_mask[indices_in_tail] = True
